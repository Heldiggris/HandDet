# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: handsDet/framework/tool/calculator_graph_template.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


from handsDet.framework import calculator_pb2 as handsDet_dot_framework_dot_calculator__pb2
try:
  handsDet_dot_framework_dot_calculator__options__pb2 = handsDet_dot_framework_dot_calculator__pb2.handsDet_dot_framework_dot_calculator__options__pb2
except AttributeError:
  handsDet_dot_framework_dot_calculator__options__pb2 = handsDet_dot_framework_dot_calculator__pb2.handsDet.framework.calculator_options_pb2
from handsDet.framework.deps import proto_descriptor_pb2 as handsDet_dot_framework_dot_deps_dot_proto__descriptor__pb2


DESCRIPTOR = _descriptor.FileDescriptor(
  name='handsDet/framework/tool/calculator_graph_template.proto',
  package='handsDet',
  syntax='proto2',
  serialized_options=_b('\n\032com.google.handsDet.protoB\022GraphTemplateProto'),
  serialized_pb=_b('\n8handsDet/framework/tool/calculator_graph_template.proto\x12\thandsDet\x1a$handsDet/framework/calculator.proto\x1a/handsDet/framework/deps/proto_descriptor.proto\"\xb8\x01\n\x12TemplateExpression\x12\r\n\x05param\x18\x01 \x01(\t\x12\n\n\x02op\x18\x02 \x01(\t\x12*\n\x03\x61rg\x18\x03 \x03(\x0b\x32\x1d.handsDet.TemplateExpression\x12\x0c\n\x04path\x18\x04 \x01(\t\x12\x38\n\nfield_type\x18\x05 \x01(\x0e\x32$.handsDet.FieldDescriptorProto.Type\x12\x13\n\x0b\x66ield_value\x18\x07 \x01(\t\"x\n\x17\x43\x61lculatorGraphTemplate\x12\x30\n\x06\x63onfig\x18\x01 \x01(\x0b\x32 .handsDet.CalculatorGraphConfig\x12+\n\x04rule\x18\x02 \x03(\x0b\x32\x1d.handsDet.TemplateExpression\"\x96\x01\n\x10TemplateArgument\x12\r\n\x03str\x18\x01 \x01(\tH\x00\x12\r\n\x03num\x18\x02 \x01(\x01H\x00\x12\'\n\x04\x64ict\x18\x03 \x01(\x0b\x32\x17.handsDet.TemplateDictH\x00\x12,\n\x07\x65lement\x18\x04 \x03(\x0b\x32\x1b.handsDet.TemplateArgumentB\r\n\x0bparam_value\"\x84\x01\n\x0cTemplateDict\x12.\n\x03\x61rg\x18\x01 \x03(\x0b\x32!.handsDet.TemplateDict.Parameter\x1a\x44\n\tParameter\x12\x0b\n\x03key\x18\x01 \x01(\t\x12*\n\x05value\x18\x02 \x01(\x0b\x32\x1b.handsDet.TemplateArgument\"\x92\x01\n\x17TemplateSubgraphOptions\x12%\n\x04\x64ict\x18\x01 \x01(\x0b\x32\x17.handsDet.TemplateDict2P\n\x03\x65xt\x12\x1c.handsDet.CalculatorOptions\x18\xf5\xfc\xbeR \x01(\x0b\x32\".handsDet.TemplateSubgraphOptionsB0\n\x1a\x63om.google.handsDet.protoB\x12GraphTemplateProto')
  ,
  dependencies=[handsDet_dot_framework_dot_calculator__pb2.DESCRIPTOR,handsDet_dot_framework_dot_deps_dot_proto__descriptor__pb2.DESCRIPTOR,])




_TEMPLATEEXPRESSION = _descriptor.Descriptor(
  name='TemplateExpression',
  full_name='handsDet.TemplateExpression',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='param', full_name='handsDet.TemplateExpression.param', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='op', full_name='handsDet.TemplateExpression.op', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='arg', full_name='handsDet.TemplateExpression.arg', index=2,
      number=3, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='path', full_name='handsDet.TemplateExpression.path', index=3,
      number=4, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='field_type', full_name='handsDet.TemplateExpression.field_type', index=4,
      number=5, type=14, cpp_type=8, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='field_value', full_name='handsDet.TemplateExpression.field_value', index=5,
      number=7, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=159,
  serialized_end=343,
)


_CALCULATORGRAPHTEMPLATE = _descriptor.Descriptor(
  name='CalculatorGraphTemplate',
  full_name='handsDet.CalculatorGraphTemplate',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='config', full_name='handsDet.CalculatorGraphTemplate.config', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='rule', full_name='handsDet.CalculatorGraphTemplate.rule', index=1,
      number=2, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=345,
  serialized_end=465,
)


_TEMPLATEARGUMENT = _descriptor.Descriptor(
  name='TemplateArgument',
  full_name='handsDet.TemplateArgument',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='str', full_name='handsDet.TemplateArgument.str', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='num', full_name='handsDet.TemplateArgument.num', index=1,
      number=2, type=1, cpp_type=5, label=1,
      has_default_value=False, default_value=float(0),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='dict', full_name='handsDet.TemplateArgument.dict', index=2,
      number=3, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='element', full_name='handsDet.TemplateArgument.element', index=3,
      number=4, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='param_value', full_name='handsDet.TemplateArgument.param_value',
      index=0, containing_type=None, fields=[]),
  ],
  serialized_start=468,
  serialized_end=618,
)


_TEMPLATEDICT_PARAMETER = _descriptor.Descriptor(
  name='Parameter',
  full_name='handsDet.TemplateDict.Parameter',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='key', full_name='handsDet.TemplateDict.Parameter.key', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='value', full_name='handsDet.TemplateDict.Parameter.value', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=685,
  serialized_end=753,
)

_TEMPLATEDICT = _descriptor.Descriptor(
  name='TemplateDict',
  full_name='handsDet.TemplateDict',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='arg', full_name='handsDet.TemplateDict.arg', index=0,
      number=1, type=11, cpp_type=10, label=3,
      has_default_value=False, default_value=[],
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[_TEMPLATEDICT_PARAMETER, ],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=621,
  serialized_end=753,
)


_TEMPLATESUBGRAPHOPTIONS = _descriptor.Descriptor(
  name='TemplateSubgraphOptions',
  full_name='handsDet.TemplateSubgraphOptions',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='dict', full_name='handsDet.TemplateSubgraphOptions.dict', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
    _descriptor.FieldDescriptor(
      name='ext', full_name='handsDet.TemplateSubgraphOptions.ext', index=0,
      number=172998261, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=True, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto2',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=756,
  serialized_end=902,
)

_TEMPLATEEXPRESSION.fields_by_name['arg'].message_type = _TEMPLATEEXPRESSION
_TEMPLATEEXPRESSION.fields_by_name['field_type'].enum_type = handsDet_dot_framework_dot_deps_dot_proto__descriptor__pb2._FIELDDESCRIPTORPROTO_TYPE
_CALCULATORGRAPHTEMPLATE.fields_by_name['config'].message_type = handsDet_dot_framework_dot_calculator__pb2._CALCULATORGRAPHCONFIG
_CALCULATORGRAPHTEMPLATE.fields_by_name['rule'].message_type = _TEMPLATEEXPRESSION
_TEMPLATEARGUMENT.fields_by_name['dict'].message_type = _TEMPLATEDICT
_TEMPLATEARGUMENT.fields_by_name['element'].message_type = _TEMPLATEARGUMENT
_TEMPLATEARGUMENT.oneofs_by_name['param_value'].fields.append(
  _TEMPLATEARGUMENT.fields_by_name['str'])
_TEMPLATEARGUMENT.fields_by_name['str'].containing_oneof = _TEMPLATEARGUMENT.oneofs_by_name['param_value']
_TEMPLATEARGUMENT.oneofs_by_name['param_value'].fields.append(
  _TEMPLATEARGUMENT.fields_by_name['num'])
_TEMPLATEARGUMENT.fields_by_name['num'].containing_oneof = _TEMPLATEARGUMENT.oneofs_by_name['param_value']
_TEMPLATEARGUMENT.oneofs_by_name['param_value'].fields.append(
  _TEMPLATEARGUMENT.fields_by_name['dict'])
_TEMPLATEARGUMENT.fields_by_name['dict'].containing_oneof = _TEMPLATEARGUMENT.oneofs_by_name['param_value']
_TEMPLATEDICT_PARAMETER.fields_by_name['value'].message_type = _TEMPLATEARGUMENT
_TEMPLATEDICT_PARAMETER.containing_type = _TEMPLATEDICT
_TEMPLATEDICT.fields_by_name['arg'].message_type = _TEMPLATEDICT_PARAMETER
_TEMPLATESUBGRAPHOPTIONS.fields_by_name['dict'].message_type = _TEMPLATEDICT
DESCRIPTOR.message_types_by_name['TemplateExpression'] = _TEMPLATEEXPRESSION
DESCRIPTOR.message_types_by_name['CalculatorGraphTemplate'] = _CALCULATORGRAPHTEMPLATE
DESCRIPTOR.message_types_by_name['TemplateArgument'] = _TEMPLATEARGUMENT
DESCRIPTOR.message_types_by_name['TemplateDict'] = _TEMPLATEDICT
DESCRIPTOR.message_types_by_name['TemplateSubgraphOptions'] = _TEMPLATESUBGRAPHOPTIONS
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

TemplateExpression = _reflection.GeneratedProtocolMessageType('TemplateExpression', (_message.Message,), dict(
  DESCRIPTOR = _TEMPLATEEXPRESSION,
  __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
  # @@protoc_insertion_point(class_scope:handsDet.TemplateExpression)
  ))
_sym_db.RegisterMessage(TemplateExpression)

CalculatorGraphTemplate = _reflection.GeneratedProtocolMessageType('CalculatorGraphTemplate', (_message.Message,), dict(
  DESCRIPTOR = _CALCULATORGRAPHTEMPLATE,
  __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
  # @@protoc_insertion_point(class_scope:handsDet.CalculatorGraphTemplate)
  ))
_sym_db.RegisterMessage(CalculatorGraphTemplate)

TemplateArgument = _reflection.GeneratedProtocolMessageType('TemplateArgument', (_message.Message,), dict(
  DESCRIPTOR = _TEMPLATEARGUMENT,
  __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
  # @@protoc_insertion_point(class_scope:handsDet.TemplateArgument)
  ))
_sym_db.RegisterMessage(TemplateArgument)

TemplateDict = _reflection.GeneratedProtocolMessageType('TemplateDict', (_message.Message,), dict(

  Parameter = _reflection.GeneratedProtocolMessageType('Parameter', (_message.Message,), dict(
    DESCRIPTOR = _TEMPLATEDICT_PARAMETER,
    __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
    # @@protoc_insertion_point(class_scope:handsDet.TemplateDict.Parameter)
    ))
  ,
  DESCRIPTOR = _TEMPLATEDICT,
  __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
  # @@protoc_insertion_point(class_scope:handsDet.TemplateDict)
  ))
_sym_db.RegisterMessage(TemplateDict)
_sym_db.RegisterMessage(TemplateDict.Parameter)

TemplateSubgraphOptions = _reflection.GeneratedProtocolMessageType('TemplateSubgraphOptions', (_message.Message,), dict(
  DESCRIPTOR = _TEMPLATESUBGRAPHOPTIONS,
  __module__ = 'handsDet.framework.tool.calculator_graph_template_pb2'
  # @@protoc_insertion_point(class_scope:handsDet.TemplateSubgraphOptions)
  ))
_sym_db.RegisterMessage(TemplateSubgraphOptions)

_TEMPLATESUBGRAPHOPTIONS.extensions_by_name['ext'].message_type = _TEMPLATESUBGRAPHOPTIONS
handsDet_dot_framework_dot_calculator__options__pb2.CalculatorOptions.RegisterExtension(_TEMPLATESUBGRAPHOPTIONS.extensions_by_name['ext'])

DESCRIPTOR._options = None
# @@protoc_insertion_point(module_scope)
